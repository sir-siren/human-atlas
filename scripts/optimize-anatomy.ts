...
const dir = new URL("../public/models/", import.meta.url),
    manifest = readAtlasManifest(new URL(name, dir));
const originals = manifest.chunks.map((c) => modelFilename(c.url));
if (manifest.optimized)
    throw new Error("Already optimized. Re-run the source converter first.");
const source = manifest.chunks.map((c) =>
    fs.readFileSync(new URL(modelFilename(c.url), dir)),
);
...
const flush = () => {
    ...
    fs.writeFileSync(new URL(modelFilename(url), dir), Buffer.concat(segments));
    ...
    if (prefix === "female") {
        ...
        for (let i = 0; i < p.vertexCount; i++) {
            ...
            if (index === undefined) {
                index = wp.length / 3;
                map.set(k, index);
                wp.push(at(pos, i * 3), at(pos, i * 3 + 1), at(pos, i * 3 + 2));
                wn.push(0, 0, 0);
            }
            remap[i] = index;
            for (let a = 0; a < 3; a++)
