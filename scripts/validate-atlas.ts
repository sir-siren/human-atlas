...
import { at, modelFilename, readAtlasManifest } from "./atlas-manifest";
import assert from "node:assert/strict";
const filename = process.argv[2] ?? "atlas.json";
const base = new URL("../public/models/", import.meta.url),
    atlas = readAtlasManifest(new URL(filename, base));
...
const files = atlas.chunks.map((c) => {
    const b = fs.readFileSync(new URL(modelFilename(c.url), base));
